		...
		std::string & content)
{
	...
	else
	{
		...
		{
			std::string::size_type pos = title.find(titleFeature_);//pos是找到的titleFeature_的位置
			if(pos != std::string::npos)
				// string search functions return npos if nothing is found
			{
				pos += titleFeature_.size();//去除掉标识的长度
				title = title.substr(pos);//将本行剩下的作为标题
				flag = true;
				break;
			}
			--count;
