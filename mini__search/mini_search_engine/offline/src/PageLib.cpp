...
void PageLib::create()
{
	std::vector<std::string> & vecFiles = dirScanner_.files();//yuliao库中所有文件的地址
	for(size_t idx = 0; idx != vecFiles.size(); ++idx)  // 00000
	{
		std::string page = fileProcessor_.process(idx, vecFiles[idx]);//vecFiles[idx]是每一篇语料文件的路径
